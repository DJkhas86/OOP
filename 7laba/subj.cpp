...
int Base::compareWave(const Base *other){
    if(this->waveLength >  other->waveLength){
        return 1;
    }
    else{
        return 0;
...
void subjList::sortList() {
    ...
    for (int i = 0; i < n - 1; i++) {
        for (int j = i + 1; j < n; j++) {
            if ((*this)[i].waveLength > (*this)[j].waveLength) {
                Item* itemToMove = removeItem(j);
                insert(itemToMove, i);
            }
        }
    }
...
void subjList::found(){
    ...
    bool found = false;
    for (int i = 0; i < n; i++) {
        if ((*this)[i].summarPower >= min && (*this)[i].summarPower <= max) {
            (*this)[i].print();
            cout << "------------------------" << endl;
            found = true;
        }
    ...
    if (!found) {
        cout << "No items in that list" << endl;
    }
}
